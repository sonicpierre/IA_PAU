...
class RC_model(object):
    ...
    def test(self, Xte, Yte, proba = False):
        ...
        if self.readout_type == 'lin': # Ridge regression        
            ...
            if proba:
                pred_class = self.readout.predict_proba(input_repr_te).max()
            else:
                pred_class = self.readout.predict(input_repr_te)
    ...
    def predict_proba(self, X):
        ...
        #   Uniquement pour les modèles utilsant le mlp
        if self.readout_type == 'mlp':
            return self.test(X, None, proba=True)
        else:
            print("Cette fonction ne marche que pour le modèle utilisant le MLP")
